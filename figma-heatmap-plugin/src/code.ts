 ...
 */
function buildImageHeat(frame: FrameNode): number[][] {
  ...
  frame.findAll(n => n.visible && "x" in n && "width" in n).forEach(node => {
    const bbox = node as SceneNode & { x: number; y: number; width: number; height: number };
...
async function predictLayout(prompt: string) {
